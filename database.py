...
class Edatabase:
    ...
    #
    def insertPunchClock(self, courseId, usrId, time, audioSrc, thumb):
        sql = "select * from punchClock where courseId = %d and usrId = %d limit 1" % (courseId, usrId)
        if self.execute(sql) != 0:
            return "existed"

        sql = "insert into punchClock(courseId, usrId, time, audioSrc, thumb) values(%d,%d,'%s','%s', %d)" % (courseId, usrId, time, audioSrc, thumb)
        self.execute(sql)
        self.con.commit()
        return "success"
    ...
    #
    def insertComment(self, punchId, usrId, time, type, content):
        sql = "insert into comment(punchId, usrId, time, type, content, thumb) values(%d,%d,'%s',%d,'%s',0)" % (punchId, usrId, time, type, content)
        self.execute(sql)
        self.con.commit()
        return "success"
